...
`src/pages/index.astro` and `src/pages/en/index.astro` both define a top-level `const CLI_VERSION` for the terminal mock (currently `0.14.0`). **Bump it on every release in lockstep with `relay-core`'s `Cargo.toml`.** The next maintainer needs to know this — do not let the constant drift. Agent-facing facts that are easy to get wrong live in `public/llms.txt` and the “For agents” section of the MCP doc; update both when a tool argument or hook signature changes.
